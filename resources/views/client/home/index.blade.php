...
@section('content')
    ...
    <div id="wp-feature-post" class="container my-4">
        <div class="box feature-post p-3 bg-light rounded shadow-sm">
            ...
            <div id="featurePostCarousel" class="carousel slide" data-bs-ride="carousel">
                    ...
                    @foreach ($specials as $index => $item)
                        <div class="carousel-item {{ $loop->first ? 'active' : '' }}">
                            ...
                            <div class="carousel-caption d-none d-md-block">
                                <a href="{{ route('client-post.show', $item->slug) }}"
                                    class="post-title text-white fw-bold">
                ...
                <!-- Controls -->
                <button class="carousel-control-prev" type="button" data-bs-target="#featurePostCarousel"
                    data-bs-slide="prev">
                    <span class="carousel-control-prev-icon" aria-hidden="true"></span>
                    <span class="visually-hidden">Previous</span>
                </button>
                <button class="carousel-control-next" type="button" data-bs-target="#featurePostCarousel"
                    data-bs-slide="next">
    ...
    <div id="wrp-content" class="container d-flex flex-wrap my-4">
        <!-- Main Content -->
        <div id="content" class="col-lg-9 col-md-12 mb-4">
            <div class="box new-post p-4 bg-white rounded shadow-sm">
                        ...
                        @foreach ($posts as $index => $post)
                            <li class="d-flex mb-4 border-bottom pb-3">
                                <a href="{{ route('client-post.show', $post->slug) }}"
                                    class="post-thumb flex-shrink-0 me-3">
                                    <img style="width: 200px; height: 150px; object-fit: cover;" class="rounded"
                                        src="{{ asset('storage/photos/19/post/' . $post->image) }}" alt="" />
                                </a>
                                <div class="more-info" style="overflow: hidden">
                                    <a href="{{ route('client-post.show', $post->slug) }}"
                                        class="post-title text-dark fw-bold mb-2 d-block">
                                        {{ $post->title }}
                                    </a>
                                    <div class="post-published small text-muted">
                                        <span class="post-author fw-semibold">{{ $post->user->fullname }}</span>
                                        <span class="post-date">•
                                            {{ $post->formatted_created_at }}</span>
                                    </div>
        ...
        <div id="side-bar" class="col-lg-4 col-md-12">
            <div class="ads mb-4 d-flex justify-content-center">
                <a href="#">
                    <img class="img-fluid rounded shadow" src="{{ asset('storage/images/logo.png') }}"
                        alt="Advertisement" />
                </a>
            </div>
